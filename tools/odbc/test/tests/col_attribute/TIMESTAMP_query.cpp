...
TEST_CASE("Test SQLColAttribute for a query that returns an TIMESTAMP", "[odbc][time]") {
	SQLHANDLE env;
	SQLHANDLE dbc;
	...
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT TIMESTAMP '1992-09-20 12:34:56.123456'"), SQL_NTS);
	...
	expected_int[SQL_DESC_CONCISE_TYPE] = ExpectedResult(SQL_TYPE_TIMESTAMP);
	...
	expected_int[SQL_DESC_TYPE] = ExpectedResult(SQL_TYPE_TIMESTAMP);
