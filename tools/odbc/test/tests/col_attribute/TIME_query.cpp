...
TEST_CASE("Test SQLColAttribute for a query that returns an TIME", "[odbc][time]") {
	SQLHANDLE env;
	SQLHANDLE dbc;
	...
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT TIME '12:34:56.123456'"), SQL_NTS);
	...
	expected_int[SQL_DESC_CONCISE_TYPE] = ExpectedResult(SQL_TYPE_TIME);
	...
	expected_int[SQL_DESC_TYPE] = ExpectedResult(SQL_TYPE_TIME);
