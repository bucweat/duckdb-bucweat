...
TEST_CASE("Test SQLColAttribute for a query that returns an DATE", "[odbc][time]") {
	SQLHANDLE env;
	SQLHANDLE dbc;
	...
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT DATE '1992-09-20'"), SQL_NTS);
	...
	expected_int[SQL_DESC_CONCISE_TYPE] = ExpectedResult(SQL_TYPE_DATE);
	...
	expected_int[SQL_DESC_TYPE] = ExpectedResult(SQL_TYPE_DATE);
